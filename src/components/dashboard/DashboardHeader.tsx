    ...
    <header className="h-14 border-b border-border bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60 flex items-center justify-between px-4 lg:px-6">
      <div className="flex items-center gap-3">
        ...
        <div className="relative hidden md:block">
          ...
          <Input
            ...
            placeholder="Search..."
            className="w-56 pl-9 h-9 bg-muted/50 border-border/50 focus:bg-background"
          />
        </div>
      </div>
      ...
      <div className="flex items-center gap-1">
        <Button variant="ghost" size="icon" className="relative h-9 w-9" aria-label="Notifications">
          ...
          <span className="absolute top-2 right-2 w-1.5 h-1.5 bg-primary rounded-full" aria-hidden="true" />
        </Button>

        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <Button variant="ghost" className="gap-2 px-2 h-9" aria-label={`${workspaceName} account menu`}>
