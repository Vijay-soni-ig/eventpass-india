      ...
      <section className="container mx-auto px-4 py-8">
        ...
        <div className="grid lg:grid-cols-3 gap-8 mt-8">
          <div className="lg:col-span-2 space-y-8">
            ...
            {participantsEnabled && !participantsQuery.isLoading && participants.length > 0 && <Card><CardHeader><CardTitle>Event participants</CardTitle></CardHeader><CardContent className="space-y-7">{Object.entries(participantGroups).map(([group, items]) => <section key={group}><h3 className="mb-3 text-sm font-semibold uppercase tracking-wide text-muted-foreground">{group}</h3><div className="grid sm:grid-cols-2 gap-4">{items.map(participant => <Link key={participant.id} to={`/event/${event.id}/participants/${participant.id}`} className="flex gap-3 rounded-xl border p-4 transition-colors hover:bg-muted/50">{participant.photoUrl ? <img src={participant.photoUrl} alt="" className="h-14 w-14 rounded-lg object-cover shrink-0" /> : <div className="h-14 w-14 rounded-lg bg-muted flex items-center justify-center shrink-0"><Users className="w-5 h-5 text-muted-foreground" /></div>}<div className="min-w-0"><p className="font-semibold truncate">{participant.name}</p>{(participant.title || participant.organization) && <p className="text-sm text-muted-foreground">{[participant.title, participant.organization].filter(Boolean).join(" · ")}</p>}{participant.bio && <p className="mt-1 text-sm text-muted-foreground line-clamp-3">{participant.bio}</p>}</div></div></Link>)}</div></section>)}</CardContent></Card>}
