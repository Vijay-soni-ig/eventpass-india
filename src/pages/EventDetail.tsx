      ...
      <section className="container mx-auto px-4 py-8">
        ...
        <div className="grid lg:grid-cols-3 gap-8 mt-8">
          <div className="lg:col-span-2 space-y-8">
            ...
            <Card><CardHeader><CardTitle>Explore more events</CardTitle></CardHeader><CardContent><div className="flex flex-wrap gap-2">
              {event.city && <Link className="rounded-full border px-3 py-1.5 text-sm hover:bg-muted" to={`/events?city=${encodeURIComponent(event.city)}`}>Events in {event.city}</Link>}
              {event.eventType && <Link className="rounded-full border px-3 py-1.5 text-sm hover:bg-muted" to={`/events?eventType=${encodeURIComponent(event.eventType)}`}>{event.eventType.charAt(0) + event.eventType.slice(1).toLowerCase()} events</Link>}
              {event.category?.id && <Link className="rounded-full border px-3 py-1.5 text-sm hover:bg-muted" to={`/events?categoryId=${encodeURIComponent(event.category.id)}`}>{event.category.name} events</Link>}
              <Link className="rounded-full border px-3 py-1.5 text-sm hover:bg-muted" to="/events">Browse all events</Link>
