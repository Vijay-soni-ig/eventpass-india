...
- Mobile: falls back to a list-form view instead of the drag canvas, with the same Bring to front / Send to back / label-visibility controls as desktop (no rotation field on mobile — position/size/stacking/label parity only).
...
A live browser-automation QA pass was run against real dev servers and a real database (see Manual QA pass below) covering create/map/drag/stacking/label-toggle/publish on the organizer side and select/reserve on the exhibitor side. **NOT TESTED**: payment completion driven through the browser (hands off to a checkout/payment-provider flow, not exercised interactively — covered only by backend integration tests), a screen-reader pass (aria attributes were verified structurally and via the accessibility tree, not with an actual screen reader), and drag/resize/rotate specifically via touch input on a real mobile device (mobile viewport rendering and mobile-editor controls were verified; touch gesture behavior was not, since the automation drives a headless Chromium mouse, not a touch pointer).
...
- No dedicated automated E2E (browser-automation) test suite exists in this repository for any feature, floor plan included; "end-to-end" verification below means real HTTP integration tests against a real database, plus a one-off manual browser-automation QA pass (see Testing status), not a suite that runs in CI.
- Rapidly adding two stalls to a draft plan (clicking "Add" twice before the first mutation's data refetch completes) can compute the same default `(x, y)` position for both, visually overlapping them on the canvas. Not a data-integrity issue — each is still a distinct, correctly-mapped `FloorPlanObject`, and dragging either apart resolves it. Found during manual QA; not fixed in this pass since it's cosmetic-only and outside the scope of what was asked.

## Manual QA pass (browser-automation, live dev servers)

A real browser session (headless, driving the actual dev server + Postgres, not a mock) was used to walk both the organizer and exhibitor flows end to end, seeding data via the real API and interacting with the real UI. This found and fixed two genuine defects that no prior test had caught:

- **Public floor-plan map was silently broken for every anonymous visitor.** `usePublicFloorPlan` called `/api/exhibitions/:id/floor-plan` (an organizer-authenticated route, mounted with `requireAuth` applied to every request regardless of which specific route matches) instead of the actual public route, `/api/public/exhibitions/:id/floor-plan`. The mismatch produced a 401 that React Query silently swallowed — the "Exhibitor Stall Layout" section on the public exhibition page simply never rendered, with no visible error. No existing test caught this because the backend integration tests correctly call the real public route directly; only a browser-driven request through the frontend's own hook exercises the URL the hook actually constructs. Fixed by correcting the URL. Verified live: the section now renders and correctly reflects live stall status (e.g. "Available (1) / Reserved (1)").
- **Stall accessible names showed the literal string "null"** (e.g. `aria-label="Stall e82f85, null, available, ₹7,500"`) whenever a stall's `stallType` was unset — a common case, since `stallType` is optional in the schema. Fixed by filtering absent parts out of the label instead of interpolating them unconditionally, in both `PublishedFloorPlan.tsx` and `FloorPlanStallPicker.tsx`.

Confirmed working end to end via this same session: draft creation, stall mapping, drag, Bring to front / Send to back (desktop and mobile), label-visibility toggle (desktop and mobile), publish, the exhibitor "Select a Stall" dialog's loading state, real stall reservation via the actual endpoint (participation moved from "approved" to showing "Proceed to Payment"), and no horizontal overflow at a 390px mobile viewport on either the organizer editor or the public map. Payment completion itself was not driven through the browser in this pass (it hands off to a checkout/payment-provider flow) — that step remains covered only by the backend integration tests (`phase29_floorPlanBookingIntegration.test.ts`).
