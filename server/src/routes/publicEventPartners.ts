...
router.get("/events/:id/partners/specialized", async (req, res) => {
  ...
  const partners = await prisma.eventParticipant.findMany({
    where: { eventId: event.id, participantType: "PARTNER", status: "ACTIVE", isPublic: true, archivedAt: null },
    orderBy: [{ sortOrder: "asc" }, { name: "asc" }],
    select: {
      id: true, name: true, title: true, organization: true, bio: true, photoUrl: true, sortOrder: true,
      website: true, email: true, phone: true,
      partnerProfile: {
        select: {
          partnerCategory: true, relationshipSummary: true, contributionSummary: true,
          engagementModel: true, displayLabel: true, displayDescription: true,
          displayOrder: true, showContact: true, showWebsite: true,
        },
      },
    },
  });
