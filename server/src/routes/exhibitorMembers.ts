...
async function getCallerRole(exhibitorBusinessId: string, userId: string) {
  const membership = await prisma.exhibitorMembership.findFirst({
    where: { exhibitorBusinessId, userId, status: "active" },
    select: { role: true },
  });
...
// My own memberships, across every exhibitor business I belong to.
...
// Full roster of one exhibitor business — any active member may view it.
router.get("/:exhibitorBusinessId", async (req, res) => {
  ...
  const members = await prisma.exhibitorMembership.findMany({
    where: { exhibitorBusinessId: req.params.exhibitorBusinessId },
    orderBy: { createdAt: "asc" },
  });
  res.json({ members });
});
...
router.patch("/member/:id", async (req, res) => {
  ...
  const updated = await prisma.exhibitorMembership.update({ where: { id: target.id }, data: parsed.data });
...
router.delete("/member/:id", async (req, res) => {
  ...
  await prisma.exhibitorMembership.delete({ where: { id: target.id } });
  res.status(204).end();
});
