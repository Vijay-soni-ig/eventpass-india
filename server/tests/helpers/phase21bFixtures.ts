...
/** Phase 21B test fixtures — small helpers for the exhibitor payment-retry, scanner, and booking-idempotency fixes. */
...
export async function signupUser(baseUrl: string, email: string, fullName: string, userType: "exhibitor" | "visitor" = "exhibitor") {
  const signup = await fetch(`${baseUrl}/api/auth/signup`, {
    ...
    body: JSON.stringify({ email, password: TEST_PASSWORD, fullName, userType }),
  }).then((r) => r.json());
...
export async function bookTicket(baseUrl: string, buyerToken: string, exhibitionId: string, ticketTypeId: string, attendeeEmail: string, idempotencyKey?: string) {
  const res = await fetch(`${baseUrl}/api/bookings/tickets`, {
    method: "POST",
    headers: { "Content-Type": "application/json", Authorization: `Bearer ${buyerToken}`, ...(idempotencyKey ? { "Idempotency-Key": idempotencyKey } : {}) },
...
/** Deletes old unlinked payments left by the documented booking/idempotency race. */
